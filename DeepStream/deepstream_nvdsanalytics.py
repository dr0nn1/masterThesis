...
OSD_PROCESS_MODE= 0
OSD_DISPLAY_TEXT= 1
classes_str= ["Person", "Falling", "In_water"]
    ...
    while l_frame:
        ...
        while l_user:
            ...
            try:
                ...
                break
        #print(f'Number of skateboard = {obj_counter[0]}')
        ...
        nowTime = time.time()
        if (timeout+(2)) < (nowTime) and FALLEN:
            FALLEN = False
            print("Time reset")
        ...
        if FALLEN:  
            ...
            py_nvosd_text_params.display_text = 'Person in water!!!!'

        else:             
            FPS = fps_streams["stream{0}".format(frame_meta.pad_index)].fps()
            py_nvosd_text_params.display_text = f'FPS = {FPS}'
...
# for RTSP ###################
    ...
    sink.set_property('sync', 0) #was 1
    ...
    streammux.set_property('batched-push-timeout', (40000)) #was 4000000 something
    
    #streammux.set_property('sync-inputs', 1)
    
    pgie.set_property('config-file-path', "config_infer_primary_yoloV4_tiny.txt")
    ...
    config.read('dsnvanalytics_tracker_config_deepsort.txt')
    config.sections()
    ...
    for key in config['tracker']:
        ...
        if key == 'display-tracking-id' :
            tracker_display_tracking = config.getint('tracker', key)
            tracker.set_property('display_tracking_id', tracker_display_tracking)

    print("Adding elements to Pipeline \n")
    pipeline.add(pgie) #yolo
    pipeline.add(tracker) #deepsort
    pipeline.add(nvanalytics) #linecrossing
    pipeline.add(tiler)
    ...
    print(
        "\n *** DeepStream: Launched RTSP Streaming at rtsp://10.0.0.202:%d/test ***\n\n"
        % rtsp_port_num
    )
